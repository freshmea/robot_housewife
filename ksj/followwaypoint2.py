...
class ClientFollowPoints(Node):
    ...
    def start(self):
        ...
        if param == 'go2':
            print('success')
            self.set_parameters([rclpy.parameter.Parameter('go_turtle', rclpy.Parameter.Type.STRING, 'stop')])
            ...
            #rgoal.pose.position.x = 0.15
            #rgoal.pose.position.y = -0.37
           
            rgoal.pose.position.z = -0.00137
            rgoal.pose.position.x = 0.112
